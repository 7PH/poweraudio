...
export class Node extends DisplayObject {
    ...
    update(delta: number) {
        ...
        const coef: number = Math.max(0.1, this.stage.viz.waveform.averageGain * 8);
    ...
    redraw() {
        ...
        // chose color
        let avg: number = 0.5 + Math.min(8, 64 * this.stage.viz.waveform.averageGain) / 16;
        ...
        this.graphics.clear();
        // aura
        const auraRadius = (this.radius + this.stage.viz.waveform.averageGainFirstOrder * 4) * 6 * (1 - Math.exp(- 3 * this.stage.viz.waveform.averageGain));
        this.graphics.lineStyle(4, this.stage.viz.waveform.averageGain > 0.15 ? this.color : grayscale, 0.3);
        this.graphics.drawCircle(0, 0, auraRadius);
        // node
        this.graphics.lineStyle(0);
        this.graphics.beginFill(this.stage.viz.waveform.averageGain > 0.15 ? this.color : grayscale);
        this.graphics.drawCircle(0, 0, this.radius + this.stage.viz.waveform.averageGainFirstOrder * 4);
