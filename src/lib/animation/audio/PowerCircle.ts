...
export class PowerCircle extends DisplayObject {
    ...
    public update(delta: number) {
        ...
        this.lineWidth = 1 + this.stage.viz.waveform.averageGainLinearized * 8;
        this.filter.blur = Math.floor(0.2 + 4 * Math.exp(- 24 * this.stage.viz.waveform.averageGainLinearized));
        this.radius = this.baseRadius + 100 * this.stage.viz.waveform.averageGainLinearized;

        this.setForce("main", {
            x: (this.targetPosition.x - this.position.x) * this.stage.viz.waveform.averageGainFirstOrder * 16,
            y: (this.targetPosition.y - this.position.y) * this.stage.viz.waveform.averageGainFirstOrder * 16,
        });
        ...
        if (Date.now() > 1400 + this.lastUpdateRandomRotation) {
            this.targetRotation = this.stage.viz.waveform.averageGainFirstOrder * 2 *(Math.random() - .5);

            if (Math.random() < 0.05 && this.stage.viz.waveform.averageGainFirstOrder >= .25) {
        ...
        // rotation
        this.rotationAcceleration = (this.targetRotation + this.targetBaseRotation - this.rotation) * this.stage.viz.waveform.averageGainFirstOrder * 16;
    ...
    private updatePosition() {
        ...
        const radius: number = this.stage.viz.waveform.averageGainFirstOrder * 256;
    ...
    private redraw() {
        ...
        // draw waveform
        const wave: Float32Array = this.stage.viz.waveform.timeDomainDataSmooth;
